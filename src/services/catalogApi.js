...
const DEMMA_EXTRA_CATEGORY_ALIASES = {
  Pannolini: [
    "panolini",
    "panolino",
    "pannolinni",
...
const DEMMA_EXTRA_LIKELY_TERMS = Object.values(DEMMA_EXTRA_CATEGORY_ALIASES).flat();

Object.entries(DEMMA_EXTRA_CATEGORY_ALIASES).forEach(([category, aliases]) => {
  const current =
    CATALOG_CONFIGS[CATALOG_KEYS.DEMMA].categoryAliases[category] || [];
...
CATALOG_CONFIGS[CATALOG_KEYS.DEMMA].extraLikelyTerms = [
  ...new Set([
    ...(CATALOG_CONFIGS[CATALOG_KEYS.DEMMA].extraLikelyTerms || []),
    ...DEMMA_EXTRA_LIKELY_TERMS,
  ]),
];
...
function productMatchesCategory(product, category, config, rawQuery = "") {
  ...
  for (const term of aliasTerms) {
    ...
    const aliasTokens = tokenize(normalizedTerm).filter((token) => token.length >= 3);
    if (aliasTokens.some((token) => fuzzyTokenMatch(token, haystackTokens))) {
      return true;
    }
  }
    ...
    if (fallbackTokens.length > 0) {
      results = products
        .map((product) => {
          ...
          const haystackTokens = tokenize(haystack);
          const matchedTokens = fallbackTokens.filter(
            (token) => haystack.includes(token) || fuzzyTokenMatch(token, haystackTokens)
          );
